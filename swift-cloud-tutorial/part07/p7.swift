...
{
  sh @seed_script "-r" 2000000 "-n" nseeds stdout=@out;
}

app (file out) genbias (int bias_range, int nvalues, file bias_script)
{
  sh @bias_script "-r" bias_range "-n" nvalues stdout=@out;
}
...
{
  sh @sim_script "-t" timesteps "-r" sim_range "-B" @bias_file "-x" scale
           "-n" sim_count "-S" @seed_file stdout=@out stderr=@log;
}

app (file out, file log) analyze (file s[], file stat_script)
{
  sh @stat_script filenames(s) stdout=@out stderr=@log;
}
